...
class Trainer:
    ...
    def train(self, save_dir, batch_gen, num_epochs, batch_size, learning_rate, device):
        ...
        torch.save(self.model.state_dict(), save_dir + "/epoch-" + str(num_epochs) + ".model")
        torch.save(optimizer.state_dict(), save_dir + "/epoch-" + str(num_epochs) + ".opt")
        self.logger.info(f"Saved final model checkpoint to {save_dir}")
    ...
    def predict(self, model_dir, results_dir, features_path, vid_list_file, epoch, actions_dict, device, sample_rate, feature_transpose=False, map_delimiter=' '):
        ...
        with torch.no_grad():
            ...
            for vid in list_of_vids:
                if 'tent' in features_path or 'tent' in vid:
                    vid_name = vid[:-4] if vid.endswith('.txt') else vid
                else:
                ...
                # Save predicted progress values (ADD THIS)
                progress_pred_np = progress_predictions[-1].cpu().numpy().squeeze(0)  # (num_classes, num_frames)
                np.save(results_dir + "/" + f_name + "_progress.npy", progress_pred_np)
