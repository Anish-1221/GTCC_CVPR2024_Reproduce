...
# --- CONFIGURATION ---
EXP_FOLDER = '/vision/anishn/GTCC_CVPR2024/output/multi-task-setting/V6___LAV_egoprocel'
# CORRECTED: Point to the actual directory with .npy files based on your screenshot
INPUT_FEAT_DIR = '/vision/anishn/GTCC_Data_Processed_1fps/egoprocel/frames/'
OUTPUT_DIR = '/vision/anishn/GTCC_Data_Processed_1fps/egoprocel/lav_aligned_features/'

def initialize_model(device):
    """Initialize and load the trained LAV model"""
    ...
    model, epoch, loss, _, _ = ckpt_restore_mprong(
        ckpt_path,
        num_heads=num_heads,
        dropout=False,  # LAV doesn't use dropout layers
        device=device
    )
...
def extract_features():
    ...
    with torch.no_grad():
        for idx, f_name in enumerate(tqdm(feat_files, desc="Extracting LAV Aligned Features")):
            try:
                # Load raw features
...
if __name__ == "__main__":
    logger.info("="*80)
    logger.info("LAV Aligned Feature Extraction")
    logger.info("="*80)
